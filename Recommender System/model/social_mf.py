...
if __name__ == '__main__':
    ...
    for i in range(1):
        print('the %dth cross validation training' % i)
        tcsr.train_model(i)
        rmse, mae, res = tcsr.predict_model()
        rmses.append(rmse)
        maes.append(mae)
    # rmse_avg = sum(rmses) / 5
    # mae_avg = sum(maes) / 5
    ...
    # print("the average of rmses is %s " % rmse_avg)
    # print("the average of maes is %s " % mae_avg)
    rmse, mae, res_1 = tcsr.predict_model()
    pred_pre = pd.DataFrame(res_1)
    pred_pre.to_csv("C:/Users/user/Desktop/Bachelor Project/dataset/filmtrust/Prediction Shift/Social_MF_bandwagon_0.06.txt", sep=' ', index=False, header=False)
